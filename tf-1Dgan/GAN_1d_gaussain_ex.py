...
# p_d(x) pdata(x) that we use to discriminate
class DataDistribution(object):
    """
    Class to generate the distribution of target data that we want to use.
    """
...
class NoiseDistribution(object):
    """
    class to generate the "noise distribution" that is fed as an input to the generator
    """
    ...
    def sample(self, N):
        ...
        noise  = np.random.random(N) * (float(self.range) / N)
        samples = np.linspace(-self.range, self.range, N) + noise
        return samples
...
def generator(x, n_hidden=32):
    """
    Generator network parametrization (fully connected)
    :param x: generator input x
    :param n_hidden: number of nodes
    :return:
    """

    # initializers
    w_init = tf.truncated_normal_initializer(stddev=2) #weights
    b_init = tf.constant_initializer(0.) #bias
...
def discriminator(x, n_hidden=32):
    """
    Definition of discriminator fully connected neural net model
    :param x: discriminator input X
    :param n_hidden: number of nodes in layer
    :return:
    """
...
def optimizer(loss, var_list, num_decay_steps=400, initial_learning_rate=0.03):
    """
    Definition of optimizer parameters
    :param loss:
    :param var_list:
...
class ResultPlot(object):
    ...
    def show_results(self, db_init, db_pre_trained, db_trained, pd, pg, save_img=True):
        ...
        plt.hist(pd, bins = 50, alpha = 0.5, density = True, label = 'pdata')
        plt.hist(pg, bins = 50, alpha = 0.5, density = True, label = 'pmodel')
    ...
    def show_plot_seaborn(self, pd, pg, save_img=True):
        ...
        sns.distplot(pg, bins = 100, kde = True, norm_hist = True, label ="generator")
        sns.distplot(pd, bins = 100, kde = True, norm_hist = True, label = "data")
        plt.grid(False)
...
def main():
    ...
    pd, _ = np.histogram(d, bins=100, density=True)
    ...
    pg, _ = np.histogram(g, bins=100, density=True)
    print("pg: ", pg)
    print("pd: ", pd)
    #plt.hist(pg, label = 'generator')
    #plt.hist(pd, label = 'discriminator')
    #plt.show()
    # plot results
